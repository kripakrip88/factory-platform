...
function inject_dobor_styles() {
	...
	.dobor-wrap .seg .dirflip.on{background:var(--accent);color:#fff;border-color:var(--accent)}
...
function init_dobor(page) {
		...
		if (v.length >= 2) {
			...
			for (let s = 0; s < segs.length; s++) {
				if (isFold(s)) {
					...
					const side = (bendAngle(s) > 0 ? -1 : 1) * (segs[s].foldFlip ? -1 : 1);
	...
	G("db_segList").addEventListener("click", (e) => {
		...
		else if (e.target.dataset.flip != null) { const i = +e.target.dataset.flip; segs[i].foldFlip = !segs[i].foldFlip; render(); } // подгиб над/под предыдущей полкой
	});
