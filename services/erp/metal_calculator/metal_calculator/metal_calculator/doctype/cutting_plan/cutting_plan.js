// Раскрой металла — форма плана: кнопка «Рассчитать раскрой» + отрисовка карт НА КЛИЕНТЕ.
// Карты рисуем из JSON (frm.doc.result_html), т.к. Frappe-санитайзер вырезает fill/style у SVG.
...
function render_result(frm) {
	...
	if (!payload || !payload.result) {
		field.$wrapper.html(`<div class="text-muted">${__("Задайте детали и размеры заготовки, затем «Рассчитать раскрой».")}</div>`);
		return;
	}
	const meta = payload.meta, result = payload.result;
	const html = meta.cut_type === "Листовой" ? render_sheet(meta, result) : render_linear(meta, result);
	field.$wrapper.html(`<div class="cut-result">${html}</div>`);
}

function render_linear(meta, result) {
	const esc = frappe.utils.escape_html;
	let out = `<div class="cut-summary">${__("Хлыстов всего")}: <b>${result.total_stock}</b> · ${__("Отход")}: <b>${result.waste_percent}%</b> · ${__("хлыст")} ${g(meta.stock_length)} ${__("мм")}, ${__("рез")} ${g(meta.kerf)} ${__("мм")}</div>`;
	for (const grp of result.groups) {
		const title = `${grp.profile_type || ""} ${grp.size_label || ""}`.trim();
		if (grp.error) { out += `<div class="cut-group cut-error"><b>${esc(title)}</b>: ⚠️ ${esc(grp.error)}</div>`; continue; }
		let rows = "";
		for (const p of grp.patterns) {
			const pieces = p.pieces.map((x) => g(x)).join(" + ");
			rows += `<div class="cut-pat">[${pieces} <span class="cut-w">+ ${g(p.waste)} ${__("отход")}</span>] ×${p.count}</div>`;
		}
		out += `<div class="cut-group"><div class="cut-gtitle">${esc(title)} — ${grp.stock_count} ${__("хлыст(ов)")}, ${__("отход")} ${grp.waste_percent}%</div>${rows}</div>`;
	}
	return out;
}

function render_sheet(meta, result) {
	const esc = frappe.utils.escape_html;
	let out = `<div class="cut-summary">${__("Листов всего")}: <b>${result.total_stock}</b> · ${__("Отход")}: <b>${result.waste_percent}%</b> · ${__("лист")} ${g(meta.sheet_length)}×${g(meta.sheet_width)} ${__("мм")}, ${__("рез")} ${g(meta.kerf)} ${__("мм")}</div>`;
	for (const grp of result.groups) {
		const title = `${__("Лист")} ${grp.size_label || ""} ${__("мм")}`.trim();
		if (grp.error) { out += `<div class="cut-group cut-error"><b>${esc(title)}</b>: ⚠️ ${esc(grp.error)}</div>`; continue; }
		out += `<div class="cut-group"><div class="cut-gtitle">${esc(title)} — ${grp.sheet_count} ${__("лист(ов)")}, ${__("отход")} ${grp.waste_percent}%</div>`;
...
function svg_sheet(sl, sw, placements, max_w = 520) {
	...
	placements.forEach((p, i) => {
		...
		if (w > 34 && h > 14) {
			parts += `<text x="${(x + w / 2).toFixed(1)}" y="${(y + h / 2 + 4).toFixed(1)}" text-anchor="middle" font-size="11" fill="#fff">${label}</text>`;
		}
	});
