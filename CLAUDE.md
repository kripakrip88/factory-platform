...
9. Перед любой правкой UI темы `saas_theme` — читать `docs/saas-theme-design.md`
   ...
   и регрессиях.
10. n8n-воркфлоу версионируются через экспорт в git (`n8n export:workflow`),
    ручная правка напрямую в БД n8n запрещена (кроме аварийной разблокировки с
    последующим экспортом в git). n8n 2.25 — модель draft/published: активна
    опубликованная версия, после импорта обязателен `publish:workflow` + restart.
    Подробности: `experiments/email-classifier/n8n-README.md`.
