...
Цветной активный модуль: `.fp-module-item.active` — фон `rgba(77,148,255,0.18)
!important` (тонировка под `--primary`, не плоский белый) + иконка `#6EAAFF`.
`!important` обязателен — без него ядро перебивает фон.
