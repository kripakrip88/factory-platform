...
4. **assets_json в Redis (критично)**: Frappe держит карту хэшей бандлов
   (`assets_json`) в `redis_cache`. При сборке образа Redis недоступен —
   карта не обновляется и остаётся со СТАРЫМИ хэшами. Тогда backend рендерит
   boot со ссылками на несуществующие `desk.bundle.<hash>.css` → 404, ядро
   Frappe грузится без стилей (гигантские иконки, рассыпанная вёрстка форм).
   `bench clear-cache` этот кэш НЕ чистит. Деплой делает `redis-cli FLUSHALL`
   на `redis_cache` после рестарта (чистый кэш, безопасно). Прецедент 2026-06-13.
