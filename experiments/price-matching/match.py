...
ABBR = [
    ...
    (r"\bтр\.?\b", "труба"),
    # Множественное число: «Трубы профильные», «Уголки», «Швеллеры» — так
    # называются разделы каталога, и позиции внутри наследуют название.
    (r"\bтрубы\b", "труба"),
    (r"\bуголки\b", "уголок"),
    (r"\bшвеллеры\b", "швеллер"),
    # «Угол» вместо «Уголок» — так пишет Феррум во всём каталоге. Граница слова
    # после «угол» не даёт задеть само «уголок».
    (r"\bугол\b", "уголок"),
    (r"\bуг\.?\b", "уголок"),
    (r"\bшв\.?\b", "швеллер"),
    (r"\bдвут\.?\b", "двутавр"),
...
def normalise(raw):
    ...
    # «10мм» — между цифрой и «мм» нет границы слова, и число не находится.
    # Отделяем единицы от числа: мм, см, м, т, кг, шт.
    s = re.sub(r"(?<=\d)\s*(мм|см|кг|шт)\b", r" \1", s)
    s = re.sub(r"[;,]", " ", s)
...
# Виды проката, которые поставщики возят, а у нас в справочнике их нет вообще.
# Разборщик должен говорить это прямо, а не «вид не опознан»: разница между
# «не понял строку» и «понял, но такого у нас нет» — это разница между
# ошибкой разбора и задачей завести номенклатуру.
MISSING_TYPES = [
    ("Полоса", r"\bполоса\b"),
    ("Катанка", r"\bкатанка\b"),
    ("Сетка", r"\bсетка\b"),
    ("Канат", r"\bканат\b"),
    ("Проволока", r"\bпроволока\b"),
    ("Шпунт", r"\bшпунт\b"),
]


def detect_missing_type(text):
    for name, pattern in MISSING_TYPES:
...
class Matcher:
    ...
    def match(self, raw):
        ...
        result["type"] = kind
        if not kind:
            missing = detect_missing_type(text)
            if missing:
                result["type"] = missing
                result["status"] = "вида нет в справочнике"
            else:
        ...
        nums, kind = self._pmk_reconcile(kind, nums)
        result["nums"], result["type"] = nums, kind
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _pmk_reconcile(kind, nums):
        """Поправки, которые видны только на живых прайсах.

        Все три случая — из настоящих каталогов, не выдуманы.
        """
        # Круглая труба не бывает с тремя размерами: если их три, поставщик
        # просто не написал слово «профильная». Так пишет половина каталогов.
        if kind == "Труба круглая" and len(nums) >= 3:
            kind = ("Труба профильная квадратная" if nums[0] == nums[1]
                    else "Труба профильная прямоугольная")

        # «Квадрат 10х10» — это наш «10»: сторона названа дважды.
        if kind == "Квадрат" and len(nums) == 2 and nums[0] == nums[1]:
            nums = nums[:1]

        # «Уголок равнополочный 20 х 4» — два числа вместо трёх: полка названа
        # один раз. Достраиваем до 20x20x4.
        if kind == "Уголок равнополочный" and len(nums) == 2:
