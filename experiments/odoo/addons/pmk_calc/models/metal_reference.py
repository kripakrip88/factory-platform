...
class MetalSheet(models.Model):
    ...
    Оцинкованный называется просто «Оцинкованный», без слова «тонколистовой»:
    толстолистовой оцинковки не бывает в принципе, а имя записи собирается как
    «Лист {вид} {толщина} мм» — лишнее слово делало запись нечитаемо длинной
    (решение владельца 21.09.2026). Толщины 0,35-1,0 мм с шагом по сортаменту,
    включая 0,65.

    Масса оцинкованного считается КАК У ОБЫЧНОГО ЛИСТА — «толщина × 7.85», без
    веса цинкового слоя. Это решение владельца (21.09.2026), а не недосмотр:
    по ГОСТ 14918 покрытие добавило бы 100-275 г/м² в зависимости от класса, но
    заводу такая точность не нужна, а лишняя переменная в расчёте веса планки
    только запутала бы. Не «уточнять» это без прямой просьбы.
    """
...
class MetalFastener(models.Model):
    ...
    # ⚠️ Массы в поставке модуля ОРИЕНТИРОВОЧНЫЕ — сняты со справочных таблиц
    # ГОСТ и годятся для прикидки. Перед боевым использованием выверить по
    # прайсу поставщика: у болтов разных производителей масса гуляет из-за
    # покрытия и допусков, а на тысяче метизов это уже заметные килограммы.
...
class PaintCoating(models.Model):
    ...
    # ⚠️ Расходы в поставке модуля ОРИЕНТИРОВОЧНЫЕ. Реальный расход зависит от
    # способа нанесения (кисть, валик, безвоздушное распыление), температуры и
    # шероховатости — разброс достигает полутора раз. Выверять по ТУ на
    # конкретную краску.
    name = fields.Char("Наименование", required=True)
