...
class DoborOrder(models.Model):
    ...
    def _sheet_html(self):
        ...
        def mps(thickness):
            # Ищем ОЦИНКОВКУ, а не гладкий лист: доборку гнут из неё, и в выборе
            # позиции с 21.09.2026 ничего другого нет. Раньше здесь стоял
            # «Гладкий», и числа сходились случайно — массы обоих видов
            # посчитаны одной формулой «толщина × 7.85». Стоило когда-нибудь
            # уточнить массу одного из видов, и печатный лист начал бы врать
            # молча, оставаясь правдоподобным.
            row = sheets.search(
                [("sheet_type", "=", "Оцинкованный"), ("thickness_mm", "=", thickness)], limit=1)
...
class DoborOrderLine(models.Model):
    ...
    # по их толщине.
    # В выборе ТОЛЬКО оцинковка (решение владельца 21.09.2026). Прежний отбор
    # «всё тоньше 2 мм» пускал в список ещё и гладкий лист 1-2 мм, из которого
    # доборку не гнут — лишние строки в выпадашке, где каждая ошибка уезжает в
    # вес и в цену. Все девять заведённых позиций и так на оцинковке
    # (проверено на стенде), поэтому ограничение ничего не ломает: домен
    # сужает выбор, но уже выбранное у старых записей не трогает.
