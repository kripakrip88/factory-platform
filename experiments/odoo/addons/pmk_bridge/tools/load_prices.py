...
ЧТО СЧИТАЕМ И ИЗ ЧЕГО
    ...
    В базе цена лежит С НДС, как в бумаге поставщика (решение владельца
    24.09.2026): сверять с прайсом глазами важнее, чем видеть чистую сумму.

    руб за метр   = цена_с_НДС / 1000 * кг_в_метре
    руб за штуку  = цена_с_НДС / 1000 * кг_в_штуке   (лист)
    руб за кг     = цена_с_НДС / 1000                (метизы, ЛКП)
...
def print_first_rows(winners, book, limit=10):
    ...
    print("  %-22s %-26s %10s %10s %8s %9s %7s" % (
        "позиция прайса", "справочник", "руб/т НДС", "в базу", "кг/ед", "цена/ед", "ед"))
    ...
    # Печатаем ровно тот счёт, который делает загрузчик. Раньше здесь стояло
    # «руб/т без НДС», хотя деление уже отключено: отчёт называл число тем,
    # чем оно не является, и проверяющий человек сверял бы не ту величину.
    print("\n  Проверка первой строки руками: %.2f руб/т с НДС, как в бумаге;"
          % winners[0].gross.amount)
