...
class MetalSpecCost(models.Model):
    ...
    supplier_id = fields.Many2one(
        ...
        domain="[('pmk_price_supplier', '=', True)]",
    ...
    #
    # ⚠️ КОЭФФИЦИЕНТ ПРИМЕНЯЕТСЯ К ГАБАРИТНЫМ РАЗМЕРАМ, А НЕ К ЧИСТОЙ ДЕТАЛИ.
    # Менеджер вбивает в расчёт размер по спецификации: круг Ø100 записывается
    # как квадрат 100×100. То есть в весе УЖЕ сидит запас на форму — у круга
    # это 27% (7854 мм² против 10000 мм²).
    #
    # Поэтому замерные 52% сюда переносить нельзя как есть: тот замер считал
    # долю от ФАКТИЧЕСКИХ деталей, а здесь знаменатель крупнее. Для круга в
    # квадрате те же 52% факта соответствуют 66% от габарита. Применить к
    # габаритному весу замерное число — значит заложить запас дважды.
    utilization_sheet_pct = fields.Float(
        "Использование листа, %", default=52.0, digits=(5, 1),
